    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-md-9 ml-sm-auto col-lg-10 px-md-4 py-4">
              ...
              <?php
                $sql = "SELECT * FROM theatre_hall WHERE theatre_id = '$hallid'";
                $query = mysqli_query($conn, $sql);
                $row = mysqli_fetch_array($query);
              ?>
              <form action="add_hall_db.php" method="post" class="form-group mt-4">
                <input type="text" name="hall_name" value="<?=$row['hall_name']?>" class="form-control form-control-lg mt-2" />
                <input type="number" name="number_of_seat" class="form-control form-control-lg mt-2" value="<?=$row['number_of_seats']?>" />
                <button type="submit" class="btn btn-success mt-3" value="<?=$hallid?>" name="save_edit_hall">บันทึก</button>
              </form>
            </main>
        </div>
    </div>
