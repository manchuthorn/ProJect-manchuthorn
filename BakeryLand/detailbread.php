        ...
        <article style="display: flex; justify-content: center;">
            ...
            <div class="detailcake">
                ...
                <div class="details">
                    ...
                    <form method="post"
                        action="./manage/manage-cart.php?action=add&ItemID=<?= htmlspecialchars($row["ItemID"]) ?>&ItemName=<?= htmlspecialchars($row["ItemName"]) ?>&BasePrice=<?= htmlspecialchars($row["BasePrice"]) ?>">
                        ...
                        <strong>Total: <span id="totalPrice">
                                <?php
                                if (isset($existingCartItem)) {
                                    echo number_format($existingCartItem['TotalPrice'], 2);
                                } else {
                                    echo number_format(isset($discountedPrice) ? $discountedPrice : $row["BasePrice"], 2);
                                }
                                ?>
                            </span> ฿</strong><br><br>
                        <input type="hidden" id="totalprice" name="totalPrice"
                            value="<?= isset($existingCartItem) ? htmlspecialchars($existingCartItem['TotalPrice']) : htmlspecialchars($row["BasePrice"]) ?>">
                        <?php
                        if (empty($_SESSION["userid"])) {
